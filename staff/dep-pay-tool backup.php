    ...
    <div class="row">
        <div class="col col-sm-12 col-md-4 col-lg-4 ">
            ...
            if(isset($_POST['switch_menu1'])){
                ...
                $sql_input_data_user="select * from member where IDMember ='$input_data' ";
                $query= mysqli_query($link, $sql_input_data_user);
                while ($rs = mysqli_fetch_array($query)) 
                    {
                        $MemberInput=array($rs['IDMember'],$rs['Firstname']);
                    }
                    // ถึงตรงนี้ 25-06-63 11.28
                for($x2=0;$x2<=$c_inc;$x2++){
                    if($x2>=$c_inc){
                        $income_dep_array[$x2]=$MemberInput;
                    }else{
